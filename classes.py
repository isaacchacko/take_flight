...
def find_image_names(paths):
	img_names_total = []
	for path in paths:
		with os.scandir(rf'{path}') as folder:
			img_names_in_folder = []
			for file in folder:
				img_names_in_folder.append(file.name)
		img_names_total.append([img_names_in_folder])
	return img_names_total

def find_img_catagories(paths):
	catagories = []
	for path in paths:
		path = path.split('/')
		path = path[-1]
		catagories.append(path)
	return catagories

# CLASSES
def create_image_dict(catagories, img_names, target_size):
	# return a dict that has the catagory as the key and the imgs as a list as the value
	img_dict = {}
	for index, catagory in enumerate(catagories):
		imgs = []
		for img_name in img_names[index]:
			imgs.append(pygame.transform.scale(pygame.image.load(img_name), (target_size)))
		img_dict[catagory] = imgs
	return img_dict

def load_images(paths, target_size):
	assert type(paths) == list
	img_names = find_image_names(paths)
	img_catagories = find_img_catagories(paths)
	img_dict = create_image_dict(img_catagories, img_names, target_size)
	return img_dict
...
class Physics():
	def __init__(self, rect, type_):
		...
		if type_ == 'player':
			self.max_vel = {'air': [[-20,20],[-10,30]], 'ground': [[-10,10],[-10,30]]}
	...
	def collisionCheck(self, terrain_rects):
	...
	def physics(self, terrain_rects):
		self.gravity()
		self.collisionCheck(terrain_rects)
		self.setAbsLoc()
		self.orientate()	

class Enemy(Physics):
	def __init__(self, rect, health, damage, speed, img_folder_paths, target_size):
		self.health = health
		self.dmg = damage
		self.speed = speed
		...
		self.img = load_images(img_folder_paths, target_size)
...
class Player(Physics):
	def __init__(self):
		...
		self.img = load_images([# ADD FOLDERS FOR THE IMAGES #], (self.width, self.height))
